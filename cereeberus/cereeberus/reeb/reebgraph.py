...
class ReebGraph(nx.MultiDiGraph):
    ...
    def thickening_distance(self, u, v):
        ...
        if self.f[u] != self.f[v]:
            raise ValueError(
                f"Vertices {u} and {v} are not at the same function value."
            )
    ...
    def add_edge(self, u, v, reset_pos=True):
        ...
        for vertex in [u, v]:
            try:
                ...
                raise ValueError(
                    f"The vertex {vertex} must be in the Reeb graph to add an edge between them."
                )
    ...
    def subdivide_edge(self, u, v, w, f_w):
        ...
        if f_w < f_lower or f_w > f_upper:
            raise ValueError(
                "The function value of the new vertex must be between the function values of the edge vertices."
            )
    ...
    def remove_regular_vertex(self, v):
        ...
        if self.up_degree(v) != 1 or self.down_degree(v) != 1:
            raise ValueError(
                "The vertex must have up degree 1 and down degree 1 to be removed as a regular vertex."
            )
    ...
    def remove_all_regular_vertices(self):
        ...
        """
        regular_vertices = [
            v for v in self.nodes if self.up_degree(v) == 1 and self.down_degree(v) == 1
        ]
    ...
    def draw(self, with_labels=True, with_colorbar=False, cpx=0.1, ax=None, **kwargs):
        """
    ...
    def slice(self, a, b, type="open", verbose=False):
        ...
        for e in e_dict:
            if e[0] in v_list and e[1] in v_list:
                ...
                if verbose:
                    print(
                        f"Adding {e_dict[e]} of edge {e} with both endpoints outside slice"
                    )
                ...
                if verbose:
                    print(
                        f"adding {e_dict[e]} of edge {e} with one endpoint outside slice"
                    )
                ...
                for i in range(e_dict[e]):
                    ...
                    if e[0] in v_list:
                        if verbose:
                            print(
                                f"Adding part of edge {e[0],e[1]} with function values {self.f[e[0]],[e[1]]} and {self.f[e[1]]}"
                            )
                            print(
                                f"This should add edge {e[0]}-{edge_name} with function values {self.f[e[0]]} and {b}"
                            )
                        ...
                        if verbose:
                            print(
                                f"Adding part of edge {e[0],e[1]} with function values {self.f[e[0]]} and {self.f[e[1]]}"
                            )
                            print(
                                f"This should add edge ({edge_name}, {e[1]}) with function values {a} and {self.f[e[1]]}"
                            )
    ...
    def get_multi_edges(self):
        ...
        mult_edges = [
            (e[0], e[1]) for e in self.edges if self.number_of_edges(e[0], e[1]) > 1
        ]
    ...
    def smoothing_and_maps(self, eps=1, verbose=False):
        ...
        min_diff = min(
            [
                new_crit_vals[i + 1] - new_crit_vals[i]
                for i in range(len(new_crit_vals) - 1)
            ]
        )
        ...
        cv_0 = (
            new_crit_vals[0] - eps
        )  # Starting value is arbitrary below the first critical value
        H_0 = self.slice(
            cv_0 - eps, cv_0 + eps, type="closed"
        )  # This should initialize empty graph
        ...
        for i in range(len(new_crit_vals)):
            ...
            for i, c in enumerate(C_edge):
                ...
                lower_vert = [
                    comp_to_new_vert_0[u] for u in np.where(overlap_down > 0)[0]
                ]
                if len(lower_vert) > 1:
                ...
                new_edge_name = (
                    lower_vert,
                    upper_vert,
                    R_eps.number_of_edges(lower_vert, upper_vert) - 1,
                )  # Because we just added the new edge, it should be the last one
    ...
    def to_mapper(self, delta=None):
        ...
        if not all([isinstance(self.f[v], int) for v in self.f]):
            raise ValueError(
                "Function values must be integers to convert to a Mapper graph."
            )
