...
class Interleave:
    ...
    def old_fit(
        ...
        while not found_max:
            ...
            if Loss <= 0:
                ...
                if verbose:
                    print(
                        f"Found valid maximum n = {N} with Loss = {Loss}. Moving on to binary search."
                    )
            else:
        ...
        if Loss > 0:
            raise ValueError(
                f"Final fit object is not an interleaving. Loss = {Loss}. N = {self.n}."
            )
    ...
    def fit(self, pulp_solver=None, verbose=False, max_n_for_error=100):
        ...
        if final_loss != 0:
            raise ValueError(
                f"Unexpected non-zero loss (Loss={final_loss}) for n={self.n}"
            )
...
class Assignment:
    ...
    def __init__(self, F, G, n=1, initialize_random_maps=False, seed=None):
        """
        Initialize the Interleave object.
        ...
        if np.abs(F.min_f() - G.min_f()) > n or np.abs(F.max_f() - G.max_f()) > n:
            raise ValueError(
                f"Function values for F and G are too far apart to interleave with n = {n}. Try initializing with a larger n."
            )
        ...
        self.I_["F"]["n"] = {}
        self.I_["F"]["n"]["V"] = LBM(
            I_n_V, self.val_to_verts["F"]["2n"], self.val_to_verts["F"]["n"]
        )
        self.I_["F"]["n"]["E"] = LBM(
            I_n_E, self.val_to_edges["F"]["2n"], self.val_to_edges["F"]["n"]
        )
        ...
        self.I_["G"]["0"] = {}
        self.I_["G"]["0"]["V"] = LBM(
            rows_dict=self.val_to_verts["G"]["n"],
        ...
        self.I_["G"]["0"]["E"] = LBM(
            I_0_E, self.val_to_edges["G"]["n"], self.val_to_edges["G"]["0"]
        )

        # Make the induced map from G_n to G_2n
        self.I_["G"]["n"] = {}
        self.I_["G"]["n"]["V"] = LBM(
            I_n_V, self.val_to_verts["G"]["2n"], self.val_to_verts["G"]["n"]
        )
        self.I_["G"]["n"]["E"] = LBM(
            I_n_E, self.val_to_edges["G"]["2n"], self.val_to_edges["G"]["n"]
        )
        ...
        for Graph, graph_name in [(self.F, "F"), (self.G, "G")]:
            for key in [
                "0",
                "n",
            ]:  # Note, we don't need to do this for 2n because the matrices are never used.
        ...
        for metagraph, name in [(self.F_, "F"), (self.G_, "G")]:
            for key in [
                ...
                self.D_[name][key]["E"] = metagraph[key].thickening_distance_matrix(
                    obj_type="E"
                )
    ...
    def B(self, graph="F", key="0"):
        ...
        """
        return (
            self.B_down_[graph][key].to_labeled_matrix()
            + self.B_up_[graph][key].to_labeled_matrix()
        )
        ...
        if phi_dict is not None:
            for thickening in ["0", "n"]:
                for obj_type in ["V", "E"]:
                    ...
                    # Now set the values
                    for key in keys:
                        self.phi_[thickening][obj_type][key] = phi_dict[thickening][
                            obj_type
                        ][key]
        ...
        if psi_dict is not None:
            for thickening in ["0", "n"]:
                for obj_type in ["V", "E"]:
                    ...
                    # Now set the values
                    for key in keys:
                        self.psi_[thickening][obj_type][key] = psi_dict[thickening][
                            obj_type
                        ][key]

    def set_single_assignment(self, obj_a, obj_b, maptype="phi", key="0", objtype="V"):
        ...
        if objtype == "V":
            ...
            if start_graph.f[obj_a] != end_graph.f[obj_b]:
                ...
                end_graph.draw()
                raise ValueError(
                    f"The objects must have the same function value. {obj_a}: {i} != {obj_b}: {j}. Nodes: {start_graph.f.keys()}, {end_graph.f.keys()}"
                )
            ...
            if start_graph.f[obj_a[0]] != end_graph.f[obj_b[0]]:
                ...
                raise ValueError(
                    f"The objects must have the same function value. {i} != {j}"
                )
            i = start_graph.f[obj_a[0]]
    ...
    def set_random_assignment(self, random_n=True, seed=None):
        ...
        if random_n:
            ...
            for obj_type in ["V", "E"]:
                self.phi_["n"][obj_type] = (
                    self.I("G", "n", obj_type)
                    @ self.phi("0", obj_type)
                    @ self.I("F", "0", obj_type).T()
                )
        ...
        if random_n:
            ...
            for obj_type in ["V", "E"]:
                self.psi_["n"][obj_type] = (
                    self.I("F", "n", obj_type)
                    @ self.psi("0", obj_type)
                    @ self.I("G", "0", obj_type).T()
                )
    ...
    def draw_all_graphs(self, figsize=(15, 10), **kwargs):
        ...
        fig, axs = plt.subplots(
            2, 3, figsize=figsize, constrained_layout=True, sharey=True
        )
    ...
    def draw_D(
        self, graph="F", key="n", obj_type="V", colorbar=True, ax=None, **kwargs
    ):
        """
    ...
    def _draw_matrix_mult(self, A, B, C, D, titles=["", "", "", ""], figsize=(12, 3)):
        """
    ...
    def parallelogram_Edge_Vert_matrix(
        self,
        maptype="phi",
        up_or_down="down",
        func_val=None,
        ...
        if up_or_down == "down":
            ...
            interleaving_map_top = self.get_interleaving_map(
                maptype, "0", "V"
            ).to_shifted_blocks(-1)
            ...
            raise ValueError(
                f"Unknown up_or_down {up_or_down}. Must be 'up', 'down', or 'both'."
            )
        ...
        if func_val is None:
            ...
            if up_or_down == "down":  # tau_i \to \sigma_i
                ...
                Top = (
                    self.get_interleaving_map(maptype, "0", "V")[func_val + 1]
                    @ self.B_up(start_graph, "0")[func_val]
                )
                Bottom = (
                    self.B_up(end_graph, "n")[func_val]
                    @ self.get_interleaving_map(maptype, "0", "E")[func_val]
                )
                Result = Top - Bottom
                ...
                raise ValueError(
                    f"Unknown up_or_down {up_or_down}. Must be 'up' or 'down'."
                )
        ...
        if draw:
            ...
            fig, axs = self._draw_matrix_mult(
                Top, Bottom, Result, Result_Dist, titles=titles
            )

        return Result_Dist

    def parallelogram_Edge_Vert(self, maptype="phi", up_or_down="down", func_val=None):
        """
    ...
    def parallelogram_matrix(
        ...
        if func_val is None:
            Top = self.get_interleaving_map(maptype, "n", obj_type) @ self.I(
                start_graph, "0", obj_type
            )
            Bottom = self.I(end_graph, "n", obj_type) @ self.get_interleaving_map(
                maptype, "0", obj_type
            )
            Result = Top - Bottom
            ...
            Top = (
                self.get_interleaving_map(maptype, "n", obj_type)[func_val]
                @ self.I(start_graph, "0", obj_type)[func_val]
            )
            Bottom = (
                self.I(end_graph, "n", obj_type)[func_val]
                @ self.get_interleaving_map(maptype, "0", obj_type)[func_val]
            )
            Result = Top - Bottom
        ...
        if draw:
            titles = ["", "", "", ""]
            titles[0] = (
                f"$M_{{{maptype_latex}_n}}^{obj_type} \\cdot I_{start_graph}^{obj_type}$"
            )
            titles[1] = f"$I_{{{end_graph}^n}} \\cdot M_{maptype_latex}^{obj_type}$"
            titles[2] = titles[0][:-1] + " - " + titles[1][1:]
            titles[3] = (
                f"$D_{{{end_graph}^{{2n}}}}^{obj_type} \\cdot ({titles[2][1:-1]})$"
            )
    ...
    def parallelogram(self, maptype="phi", obj_type="V", func_val=None):
        """
    ...
    def triangle_matrix(
        ...
        if func_val is None:
            Top = self.I(start_graph, "n", obj_type) @ self.I(
                start_graph, "0", obj_type
            )
            Bottom = self.get_interleaving_map(
                maptype=map2, key="n", obj_type=obj_type
            ) @ self.get_interleaving_map(maptype=map1, key="0", obj_type=obj_type)
            Result = Top - Bottom
            # Result = Result.to_labeled_matrix() # To make the matrix labeledmatrix
            ...
            Top = (
                self.I(start_graph, "n", obj_type)[func_val]
                @ self.I(start_graph, "0", obj_type)[func_val]
            )
            Bottom = (
                self.get_interleaving_map(maptype=map2, key="n", obj_type=obj_type)[
                    func_val
                ]
                @ self.get_interleaving_map(maptype=map1, key="0", obj_type=obj_type)[
                    func_val
        ...
        if draw:
            titles = ["", "", "", ""]
            titles[0] = (
                f"$I_{{{start_graph}^n}}^{obj_type} \\cdot I_{start_graph}^{obj_type}$"
            )
            titles[1] = (
                f"$M_{{{map2_latex}^n}}^{obj_type} \\cdot M_{{{map1_latex}}}^{obj_type}$"
            )
            titles[2] = titles[0][:-1] + " - " + titles[1][1:]
            titles[3] = (
                f"$D_{{{start_graph}^{{2n}}}}^{obj_type} \\cdot ({titles[2][1:-1]})$"
            )
            fig, axs = self._draw_matrix_mult(
                Top, Bottom, Result, Result_Dist, titles=titles
            )
    ...
    def loss_table(self):
        ...
        for maptype in ["phi", "psi"]:
            for up_or_down in ["up", "down"]:
                result = self.parallelogram_Edge_Vert(
                    maptype=maptype, up_or_down=up_or_down
                )
                loss = result
        ...
        loss_table = pd.DataFrame(
            loss_list, columns=["Dgm Type", "Req A", "Req B", "Loss"]
        )
        return loss_table
    ...
    def loss_by_block(self):
        ...
        for i in all_func_vals:
            ...
            for Graph, graph_name, maptype in [
                (self.F, "F", "phi"),
                (self.G, "G", "psi"),
            ]:
                obj_type = "V"
                if i in Graph().get_function_values():
                    loss = self.parallelogram(
                        maptype=maptype, obj_type=obj_type, func_val=i
                    )
                    # loss = result.absmax()
            ...
            for Graph, graph_name in [(self.F, "F"), (self.G, "G")]:
                ...
                if i in Graph().get_function_values():
                    loss = self.triangle(
                        start_graph=graph_name, obj_type=obj_type, func_val=i
                    )
                    # loss = result.absmax()
                    loss_list.append(loss)
                else:
            ...
            for Graph, graph_name, maptype in [
                ...
                if i in edge_vals:
                    for up_or_down in ["up", "down"]:
                        loss = self.parallelogram_Edge_Vert(
                            maptype=maptype, func_val=i, up_or_down=up_or_down
                        )
                        # loss = result.absmax()
