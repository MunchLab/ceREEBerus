    ...
    for thickening in ["0", "n"]:
        for obj_type in ["V", "E"]:
            for map_type in ["phi", "psi"]:
                ...
                M = myAssgn.get_interleaving_map(
                    maptype=map_type, key=thickening, obj_type=obj_type
                )
                ...
                for i in M.get_all_block_indices():
                    ...
                    for a in range(block_shape[0]):
                        for b in range(block_shape[1]):
                            ...
                            if a < 0 or b < 0:
                                print(
                                    f"Warning: a={a}, b={b} for block {i} in {map_type}_{thickening}_{obj_type}."
                                )
                            try:
                                M[i].array[a, b] = pulp.value(
                                    map_results[map_type + "_vars"][i][thickening][
                                        obj_type
                                    ][(a, b)]
                                )
                            except KeyError:
...
def solve_ilp(myAssgn, pulp_solver=None, verbose=False):
    ...
    phi_vars = {
        block: {
            thickening: {obj_type: {} for obj_type in ["V", "E"]}
            for thickening in ["0", "n"]
        }
    ...
    z_vars = {
        block: {
            starting_map: {obj_type: {} for obj_type in ["V", "E"]}
            ...
            for block in (func_vals[:-1] if obj_type == "E" else func_vals):
                ...
                if starting_map == "F":
                    ...
                    z_vars[block][starting_map][obj_type] = pulp.LpVariable.dicts(
                        ...
                        (
                            (a, b, c)
                            for a in range(n_rows_1)
                            for b in range(n_rowcol_1)
    ...
    for block in func_vals:
        for starting_map in ["F", "G"]:
            ...
            for up_or_down in [
                "up",
                "down",
            ]:  # deals with 1 (up, down) and 2 (up, down)

                if (
                    block == myAssgn.all_func_vals(map=starting_map)[-1]
                ):  # skip the last block for this type of diagrams
                    continue
                ...
                for i in range(shape_m_mix):
                    for k in range(shape_p_mix):
                        # inner difference
                        first_term = pulp.lpSum(
                            [map_V_vars[i, j] * bou_0[j][k] for j in range(shape_n_mix)]
                        )
                        second_term = pulp.lpSum(
                            [bou_n[i][l] * map_E_vars[l, k] for l in range(shape_o_mix)]
                        )
                ...
                for j in range(shape_n_mix):
                    prob += (
                        pulp.lpSum(map_V_vars[h, j] for h in range(shape_m_mix)) == 1
                    )

                for k in range(shape_p_mix):
                    prob += (
                        pulp.lpSum(map_E_vars[l, k] for l in range(shape_o_mix)) == 1
                    )

            for obj_type in ["V", "E"]:  # deals with 3, 4, 5, 6, 7, 8, 9, 10
                if (
                    obj_type == "E" and block == func_vals[-1]
                ):  # skip the last block for this type of diagrams. This is because we don't have an edge with the highest function value
                    continue

                # multiply inclusion matrices. Needed for the triangles
                i_n_i_0 = (
                    myAssgn.I(starting_map, "n", obj_type)[block].get_array()
                ...
                shape_o_para = (
                    myAssgn.I(other_map, "n", obj_type)[block].get_array().shape[1]
                )  # for parallelograms

                shape_n_para = (
                    myAssgn.I(starting_map, "0", obj_type)[block].get_array().shape[0]
                )  # for parallelograms

                if starting_map == "F":
                    shape_n_tri = (
                        myAssgn.psi("n", obj_type)[block].get_array().shape[1]
                    )  # for triangles
                    shape_o_tri = (
                        myAssgn.phi("0", obj_type)[block].get_array().shape[1]
                    )  # for triangles

                    shape_m_para = (
                        myAssgn.phi("n", obj_type)[block].get_array().shape[0]
                    )  # for parallelograms
                    shape_p_para = (
                        myAssgn.phi("0", obj_type)[block].get_array().shape[1]
                    )  # for parallelograms
                    ...
                    shape_n_tri = (
                        myAssgn.phi("n", obj_type)[block].get_array().shape[1]
                    )  # for triangles
                    shape_o_tri = (
                        myAssgn.psi("0", obj_type)[block].get_array().shape[1]
                    )  # for triangles

                    # changed shape_n_para = myAssgn.psi('n', obj_type)[block].get_array().shape[1] # for parallelograms

                    shape_m_para = (
                        myAssgn.psi("n", obj_type)[block].get_array().shape[0]
                    )  # for parallelograms
                    shape_p_para = (
                        myAssgn.psi("0", obj_type)[block].get_array().shape[1]
                    )  # for parallelograms
                ...
                for h in range(shape_m_tri):
                    for k in range(shape_o_tri):
                        prob += map_product_vars[block][starting_map][obj_type][
                            h, k
                        ] == float(i_n_i_0[h, k])
                ...
                for i in range(shape_m_para):
                    for k in range(shape_p_para):
                        ...
                        first_term = pulp.lpSum(
                            [
                                map_n_para_vars[i, j] * inc_0_para[j][k]
                                for j in range(shape_n_para)
                            ]
                        )
                        second_term = pulp.lpSum(
                            [
                                inc_n_para[i][l] * map_0_para_vars[l, k]
                                for l in range(shape_o_para)
                            ]
                        )
                ...
                for i in range(shape_m_tri):
                    for k in range(shape_o_tri):
                        prob += map_product_vars[block][starting_map][obj_type][
                            ...
                            z_vars[block][starting_map][obj_type][i, j, k]
                    ...
                    for j in range(shape_n_tri):
                        for k in range(shape_o_tri):
                            if starting_map == "F":
                                prob += (
                                    z_vars[block][starting_map][obj_type][i, j, k]
                                    <= psi_vars[block]["n"][obj_type][i, j]
                                )
                                prob += (
                                    z_vars[block][starting_map][obj_type][i, j, k]
                                    <= phi_vars[block]["0"][obj_type][j, k]
                                )
                                prob += (
                                    z_vars[block][starting_map][obj_type][i, j, k]
                                    >= psi_vars[block]["n"][obj_type][i, j]
                                ...
                                prob += (
                                    z_vars[block][starting_map][obj_type][i, j, k]
                                    <= phi_vars[block]["n"][obj_type][i, j]
                                )
                                prob += (
                                    z_vars[block][starting_map][obj_type][i, j, k]
                                    <= psi_vars[block]["0"][obj_type][j, k]
                                )
                                prob += (
                                    z_vars[block][starting_map][obj_type][i, j, k]
                                    >= phi_vars[block]["n"][obj_type][i, j]
    ...
    # return results
    return final_maps, status_str


##------------------- ILP Optimization with Distance Matrix -------------------##


def solve_ilp_dist(myAssgn, pulp_solver=None, verbose=False):
    """
    ...
    for obj_type in ["V", "E"]:
        for starting_map in ["F", "G"]:
            ...
            for block in (func_vals[:-1] if obj_type == "E" else func_vals):
                if starting_map == "F":
                    shape_m_tri = (
                        myAssgn.D("F", "2n", obj_type)[block].get_array().shape[0]
                    )
                else:
                    shape_m_tri = (
    ...
    for block in func_vals:
        for starting_map in ["F", "G"]:
            ...
            for up_or_down in [
                ...
                for i in range(shape_m_mix):
                    for k in range(shape_p_mix):
                        ...
                        # total expression
                        mixed_expression = pulp.lpSum(
                            dist_n_other[i][h] * (first_term - second_term)
                            for h in range(shape_m_mix)
                        )
            ...
            for obj_type in ["V", "E"]:  # deals with 3, 4, 5, 6, 7, 8, 9, 10
                ...
                # for triangles
                dist_2n_starting = myAssgn.D(starting_map, "2n", obj_type)[
                    block
                ].get_array()
                # for parallelograms
                ...
                for h in range(shape_m_tri):
                    ...
                    tri_expression = pulp.lpSum(
                        dist_2n_starting[i, h]
                        * (
                            i_n_i_0[h, k]
                    ...
                    prob += (
                        aux_vars[block][starting_map][obj_type] * 2 >= tri_expression
                    )  # ceiling of half of the expression
                ...
                for i in range(shape_m_para):
                    for k in range(shape_p_para):
                        ...
                        # total expression
                        para_expression = pulp.lpSum(
                            dist_2n_other[i][h] * (first_term - second_term)
                            for h in range(shape_m_para)
                        )
